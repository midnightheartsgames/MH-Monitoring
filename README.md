...
- **GPU (NVIDIA):** загрузка, температура, частоты ядра и памяти, VRAM, потребление, обороты
  вентилятора. Всё это читается через NVML без прав администратора.
- **CPU:** загрузка и реальная текущая частота с учётом буста (через PDH, как в диспетчере
  задач). Температура и мощность доступны для AMD Zen 1–5 через драйвер
...
- Для показаний GPU нужна видеокарта NVIDIA с установленным драйвером (`nvml.dll` поставляется
  вместе с ним).
- Для температуры и мощности CPU нужен процессор AMD Ryzen/EPYC (семейства 17h–1Ah) и
  установленный драйвер [PawnIO](https://pawnio.eu).
...
> **Ограничение.** В эксклюзивном полноэкранном режиме оверлей не виден поверх игры: окна
> пользовательского режима туда не попадают. Используйте безрамочный оконный режим.
...
│ NVML, sysinfo, PDH, PawnIO   → железо          │◄──────►│ выбор цели по окну в фокусе                 │
...
Готовый `MH-Monitoring.exe` и есть весь дистрибутив: PresentMon, модуль PawnIO, шрифты, иконка и
...
| `crates/platform` | Тонкие обёртки Win32: ETW, окна, канал, реестр, ярлыки, UAC |
| `crates/sources` | Источники данных: PresentMon, собственный ETW, NVML, sysinfo, PDH, PawnIO |
...
| `assets` | Шрифты, иконка, вложенные PresentMon и модуль PawnIO с лицензиями |
...
| [PawnIO](https://github.com/namazso/PawnIO.Modules), модуль `AMDFamily17` 0.2.11 | LGPL-2.1-or-later | [assets/pawnio/NOTICE.md](assets/pawnio/NOTICE.md) |
