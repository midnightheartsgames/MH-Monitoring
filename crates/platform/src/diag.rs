//! Короткий журнал последнего запуска: шаги со временем от старта.
//!
//! Общий для оверлея (`MH-Monitoring.log`) и службы (`service.log`): по письму пользователя
//! должно быть видно, где уходят секунды.
...
mod tests {
    ...
    fn the_journal_records_steps_with_durations() {
        let path = std::env::temp_dir()
            .join(format!("mh-platform-diag-{}", std::process::id()))
